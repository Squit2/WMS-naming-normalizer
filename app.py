...
------------------------------------------------------
# Config-storage SFTP (mapping files live here)
...
# ── SFTP config persistence layer (new) ───────────────────────────────────────
...
# Only the output-delivery keys are bridged here; config-storage keys are read
# directly from st.secrets inside sftp_config_store.py.
...
# export_payload  — bytes ready to download / upload to WMS
# proc_result     — dict with all pipeline output (persists across reruns)
# proc_file_key   — hash string used to detect file/customer changes
# saved_mapping   — user_mapping dict that was last saved to SFTP
...
def sanitise_config_key(raw_name: str) -> str:
    ...
    Strips the extension entirely (keys are extension-less).

    e.g. "../../converter.py" → "______converter"
         "Gigly Gulp 2026.csv" → "Gigly_Gulp_2026"
    """
...
def _file_key(file_bytes: bytes, customer_key: str) -> str:
    """
    Cheap fingerprint for (uploaded file, selected customer).
    Used to invalidate session_state["proc_result"] when either changes.
    """
...
# ─── LOAD SFTP CONFIGS ───────────────────────────────────────────────────────
# Done at the top of every render cycle. The result is cached (TTL 5 min) so
# this only hits the network on the first load or after a write-back.

try:
    raw_configs = fetch_all_raw_configs()
except (RuntimeError, ImportError, ValueError) as _sftp_err:
    st.error(
        "**Cannot connect to SFTP config store.**  \n"
        "Check SFTP_HOST, SFTP_USERNAME, SFTP_PASSWORD in Streamlit secrets.  \n"
...
with st.sidebar:
    ...
    uploaded_config = st.file_uploader(
        ...
        help=(
            ...
            "The filename stem becomes the customer key.  \n"
            "Maximum size: 1 MB."
        ),
    )
        ...
        if len(config_bytes) > MAX_CONFIG_BYTES:
            ...
            if safe_key == "template":
                st.error(
                    ...
                    "Rename your config file and re-upload."
                )
            else:
                # Convert to CSV string (xlsx uploads are normalised to CSV)
                try:
                    if ext == ".csv":
                        csv_string = config_bytes.decode("utf-8-sig")
                    else:
                        # Read xlsx in-memory and export as CSV
                        df_tmp = pd.read_excel(
                            BytesIO(config_bytes), dtype=str
                        )
                        csv_string = df_tmp.to_csv(index=False)

                    # Validate the config before writing to SFTP
                    ...
                    if result["success"]:
                        st.success(
                            "Config saved: **{}**".format(safe_key)
                        )
                        if safe_key != Path(uploaded_config.name).stem:
                            st.info(
                                "Key sanitised: '{}' → '{}'".format(
                                    Path(uploaded_config.name).stem, safe_key
                                )
                            )
                        if ext == ".xlsx":
                            st.info(
                                "XLSX converted to CSV before upload to SFTP."
                            )
                        # Bust the cache so the new config appears immediately
                    ...
                    st.error(
                        "Invalid config — will not upload.  \n"
                        "Fix the following error and re-upload:  \n"
                        "`{}`".format(_ve)
                    )
    ...
    if not customers:
        st.error(
            "No customer configs found on SFTP.  \n"
    ...
    if invalid_configs:
        st.warning(
            "{} invalid config(s) detected.".format(len(invalid_configs))
        )

    if st.button("Validate all configs"):
        for key in customers:
            report = all_config_reports.get(
                key, {"errors": [], "warnings": []}
            )
            if report["errors"]:
                ...
                st.warning(
                    "{}: {}".format(key, " | ".join(report["warnings"]))
                )
            else:
...
if uploaded_file and config_is_valid:
    ...
    # ── Compute fingerprint — invalidate stored results when file/customer changes
    ...
    # ── Read raw file (fast; always done so we can detect unmapped columns) ──
    tmp_path = None
    try:
    ...
    # ── Detect unmapped columns (case-insensitive, matching apply_mapping) ───
    ...
    # ── Dynamic mapping UI ───────────────────────────────────────────────────
    user_mapping = {}   # {customer_col: wms_field}

    if unmapped_cols:
        st.info(
            "**{} column(s)** in the uploaded file are not in the mapping config.  \n"
            "Assign each to a WMS field or leave it as *Ignore* to drop it.".format(
                len(unmapped_cols)
            )
        )

        # Render selectboxes in a responsive 3-column grid
        _COLS_PER_ROW = 3
    ...
    run_conversion = st.button(_btn_label, type="primary")

    if run_conversion:
        # Build the merged mapping: config mapping + UI overrides
        final_map = {**config["column_map"], **user_mapping}

        _tmp_path2 = None
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=ext) as _tmp2:
                _tmp2.write(file_bytes)
                _tmp_path2 = Path(_tmp2.name)
        ...
        # Store results so they survive reruns (widget interactions, etc.)
        st.session_state["proc_result"] = {
            ...
            "user_mapping": dict(user_mapping),   # snapshot at conversion time
        }
        # Reset any previously saved mapping for this file
        st.session_state["saved_mapping"] = {}
    ...
    if proc and st.session_state["proc_file_key"] == current_file_key:
        ...
        if conv_user_mapping:
            ...
            already_saved = st.session_state["saved_mapping"] == conv_user_mapping

            if already_saved:
                st.success(
                    "{} mapping(s) already saved to SFTP for this conversion.".format(
                ...
                st.caption(
                    "The following mappings were applied during this conversion "
                    "but are not yet in the config. Save them so future files "
                    "from this customer are mapped automatically."
                )
                st.dataframe(
                    mapping_preview, hide_index=True, use_container_width=True
                )
...
# ─── DOWNLOAD BUTTONS (outside if/else — survive reruns) ─────────────────────
...
st.caption(
    ...
    "To add a new customer, upload a mapping config (.csv or .xlsx) "
    "using the sidebar. It will be saved to the SFTP config store automatically."
)
